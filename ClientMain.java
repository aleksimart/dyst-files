...
public class ClientMain {
	...
	public static void main(String[] args) throws Exception {
		...
		// example to launch a number of concurrent clients, each doing the same
		// operations
		for (int i = 0; i < 10; i++) {
			new Thread() {
				public void run() {
	...
	public static void testClient(int cport, int timeout, File downloadFolder) {
		...
		try {
			...
			try {
				client.store(new File("Clipboard01.pdf"));
			} catch (IOException e) {
				e.printStackTrace();
			}
			...
			try {
				client.store(new File("Clipboard01.jpg"));
			} catch (IOException e) {
				e.printStackTrace();
			}
			...
			/*
			 * if (list != null) for (String filename : list) try { client.remove(filename);
			 * } catch(IOException e) { e.printStackTrace(); } try { client.remove(list[0]);
